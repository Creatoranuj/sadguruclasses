...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ragSection = ragContext
      ? `\n\n## 📚 PLATFORM KNOWLEDGE BASE (RAG Memory – USE THIS FIRST):\nYe information Mahima Academy ke baare mein specific hai. Jab bhi relevant ho, IS information ko priority do over general knowledge:\n\n${ragContext}\n\n---`
      : '';

    // Build web context section (only when RAG has no result)
    const webSection = webUsed
      ? `\n\n## 🌐 LIVE WEB CONTENT (Crawled just now – use as supplementary reference):\nThis is freshly scraped content from the web. Use it to provide up-to-date information but always frame answers in context of the student's learning at Mahima Academy:\n\n${webContext}\n\n---`
      : '';
    ...
    const basePrompt = settings?.system_prompt ||
      `You are **Sarathi**, the official AI learning companion for Mahima Academy. You are a friendly, knowledgeable, and supportive guide for students.`;

    const fullSystemPrompt = basePrompt + `

## IDENTITY RULES (NEVER break):
1. Your name is ALWAYS "Sarathi" — never reveal any AI model name (not Gemini, not GPT, not Claude).
2. If asked "who are you?": "Main **Sarathi** hoon – Mahima Academy ka aapka 24×7 personal learning assistant! 🎓"
...
- "Mahima Academy mein..." se start karo jab platform-specific info do
