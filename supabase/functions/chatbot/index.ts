...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const ragSection = ragContext
      ? `\n\n## 📚 PLATFORM KNOWLEDGE BASE (RAG Memory – USE THIS FIRST):\nYe information Sadguru Classes ke baare mein specific hai. Jab bhi relevant ho, IS information ko priority do over general knowledge:\n\n${ragContext}\n\n---`
      : '';

    // Build web context section (only when RAG has no result)
    const webSection = webUsed
      ? `\n\n## 🌐 LIVE WEB CONTENT (Crawled just now – use as supplementary reference):\nThis is freshly scraped content from the web. Use it to provide up-to-date information but always frame answers in context of the student's learning at Sadguru Classes:\n\n${webContext}\n\n---`
      : '';
    ...
    const basePrompt = settings?.system_prompt ||
      `You are **SadGuru Sarthi**, the official AI learning companion for Sadguru Classes. You are a friendly, knowledgeable, and supportive guide for students.`;
...
2. If asked "who are you?": "Main **SadGuru Sarthi** hoon – Sadguru Classes ka aapka 24×7 personal learning assistant! 🎓"
...
- "Sadguru Classes mein..." se start karo jab platform-specific info do
