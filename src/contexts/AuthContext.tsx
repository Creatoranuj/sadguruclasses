...
interface AuthContextType {
  ...
  signup: (email: string, password: string, fullName: string, goal?: string, classGrade?: string) => Promise<{ error: Error | null }>;
  logout: () => Promise<void>;
...
function makeDefaults(supabaseUser: SupabaseUser): { user: User; profile: UserProfile; role: AppRole } {
  ...
  return {
    ...
    profile: { id: supabaseUser.id, email, fullName: metaName, avatarUrl: null, mobile: null, goal: null, class_grade: null },
    role: "student",
  };
}
...
async function fetchUserData(
  ...
  const actualFetch = async () => {
    const [profileResult, roleResult] = await Promise.all([
      supabase.from("profiles").select("id, full_name, email, avatar_url, mobile, goal, class_grade").eq("id", supabaseUser.id).single().then(r => r, e => ({ data: null, error: e })),
    ...
    if (!profileData && isSignup) {
      ...
      const retry = await supabase.from("profiles").select("id, full_name, email, avatar_url, mobile, goal, class_grade").eq("id", supabaseUser.id).single();
    ...
    return {
      ...
      profile: { id: supabaseUser.id, email, fullName, avatarUrl: profileData?.avatar_url ?? null, mobile: profileData?.mobile ?? null, goal: profileData?.goal ?? null, class_grade: profileData?.class_grade ?? null },
      role,
    };
  };

  try {
    // Race against a 4-second timeout — if profile/role fetch hangs, use defaults
    const result = await Promise.race([
      actualFetch(),
      new Promise<typeof defaults>((resolve) => setTimeout(() => {
        console.warn("[AuthContext] fetchUserData timed out after 4s, using defaults");
        resolve(defaults);
      }, 4000)),
    ]);
    return result;
...
export const AuthProvider = ({ children }: { children: ReactNode }) => {
  ...
  const signup = async (email: string, password: string, fullName: string, goal?: string, classGrade?: string) => {
    try {
      const { error } = await supabase.auth.signUp({
        ...
        options: {
          ...
          data: { full_name: fullName, goal: goal || null, class_grade: classGrade || null },
        },
      });
      if (error) return { error };
      return { error: null };
