import { Suspense, lazy, memo, useState, useEffect, useCallback } from "react";
...
import { setupCapacitorBackButton } from "./lib/capacitorBackButton";
import { checkAndCleanStorage } from "./lib/storageCleanup";
...
// AuthRedirect: sends authenticated users from landing page to dashboard
const AuthRedirect = ({ element }: { element: React.ReactElement }) => {
  const { isAuthenticated, isLoading } = useAuth();
  if (isLoading) return <PageLoader />;
...
// Hook to set up Capacitor back button and run storage cleanup on mount
const useAppInit = () => {
  useEffect(() => {
    const cleanup = setupCapacitorBackButton();
          ...
          <TooltipProvider>
            ...
            <BrowserRouter>
              ...
              <Suspense fallback={<PageLoader />}>
                <Routes>
                  {/* Public Routes */}
                  <Route path="/" element={<AuthRedirect element={<Index />} />} />
                  ...
                  <Route path="/admin/monitor" element={<AdminRoute element={<AdminMonitor />} />} />
                  ...
                  <Route path="/my-courses/:courseId" element={<ProtectedRoute element={<MyCourseDetail />} />} />
                  ...
                  <Route path="/classes/:courseId/lessons" element={<ProtectedRoute element={<LessonView />} />} />
                  ...
                  {/* Quiz Routes */}
                  <Route path="/quiz/:quizId" element={<ProtectedRoute element={<QuizAttempt />} />} />
                  ...
                  <Route path="/live/:sessionId" element={<ProtectedRoute element={<LiveClass />} />} />
