...
const Courses = () => {
    ...
    try {
      ...
      if (!error && data) {
        const safeData = data.map((item: any) => ({
          ...
          target_exam: item.category || "general",
          rating: 4.8,
          duration: "0m",
          lessons_count: 0,
        }));
  ...
  const examOptions = ["Board", "JEE", "NEET", "Foundation", "UPSC", "SSC"];

  const filteredCourses = courseList.filter((c: any) => {
    const gradeMatch = selectedGrade === "all" || String(c.grade) === String(selectedGrade);
    const examMatch = selectedExam === "all" || (c.target_exam && c.target_exam === selectedExam);
