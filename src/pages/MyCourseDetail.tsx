...
const MyCourseDetail = () => {
  ...
  // Fetch sub-chapters for selected chapter
  ...
  useEffect(() => {
    if (!selectedChapterId || selectedChapterId === "__all__") { setSubChapters([]); setSelectedSubChapterId(null); return; }
    const fetchSub = async () => {
      const { data } = await supabase.from("chapters").select("*")
        .eq("parent_id", selectedChapterId).order("position", { ascending: true });
      if (data && data.length > 0) {
        const mapped: Chapter[] = data.map((ch: any) => {
          const lCount = lessons.filter(l => l.chapterId === ch.id).length;
          const cCount = lessons.filter(l => l.chapterId === ch.id && completedLessonIds.has(l.id)).length;
          return { id: ch.id, code: ch.code, title: ch.title, position: ch.position, parent_id: ch.parent_id, lessonCount: lCount, completedLessons: cCount };
        });
        setSubChapters(mapped);
      } else {
        setSubChapters([]);
      }
    };
    fetchSub();
  }, [selectedChapterId, lessons, completedLessonIds]);

  const chapterLessons = useMemo(
    () => {
      if (!selectedChapterId || selectedChapterId === "__all__") return lessons;
      if (selectedSubChapterId) return lessons.filter(l => l.chapterId === selectedSubChapterId);
      // If chapter has sub-chapters, show all lessons from this chapter AND its sub-chapters
      if (subChapters.length > 0) {
        const subIds = new Set(subChapters.map(sc => sc.id));
        return lessons.filter(l => l.chapterId === selectedChapterId || (l.chapterId && subIds.has(l.chapterId)));
      }
      return lessons.filter(l => l.chapterId === selectedChapterId);
    },
    [lessons, selectedChapterId, selectedSubChapterId, subChapters]
  );
  ...
  const selectedSubChapter = useMemo(
    () => subChapters.find(sc => sc.id === selectedSubChapterId),
    [subChapters, selectedSubChapterId]
  );
  ...
  const lessonBreadcrumbs = useMemo(() => [
    ...
    ...(selectedChapter && selectedChapter.id !== "__all__"
      ? [{ label: `${selectedChapter.code} : ${selectedChapter.title}`, ...(selectedSubChapterId ? { href: '#' } : {}) }]
      : []),
    ...(selectedSubChapter ? [{ label: selectedSubChapter.title }] : []),
  ], [course, courseId, selectedChapter, selectedSubChapter, selectedSubChapterId]);
          ...
          {selectedChapterId && !selectedLesson && (() => {
            ...
            return (
              <>
                ...
                {/* Sub-folder cards — show when chapter has sub-chapters and none selected */}
                {subChapters.length > 0 && !selectedSubChapterId && selectedChapterId !== "__all__" && (
                  <div className="px-5 pt-3 space-y-2">
                    <p className="text-xs font-semibold text-muted-foreground uppercase tracking-wider mb-2">Sub-Topics</p>
                    {subChapters.map((sc) => (
                      <ChapterCard
                        key={sc.id}
                        code={sc.code}
                        title={sc.title}
                        lectureCount={sc.lessonCount}
                        completedLectures={sc.completedLessons}
                ...
                {/* Tab bar + view toggle — hide when sub-folders are visible */}
                {(selectedChapterId === "__all__" || subChapters.length === 0 || !!selectedSubChapterId) && (<>
