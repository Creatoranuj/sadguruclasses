...
import { ArrowLeft, User, Mail, Shield, LogOut, Loader2, Phone, GraduationCap, Target } from "lucide-react";
  ...
  const handleSave = async () => {
    ...
    try {
      const { error } = await supabase
        .from('profiles').update({ full_name: nameInput, mobile: mobileInput, class_level: classLevel || null, target_exam: targetExam || null }).eq('id', profile.id);
      if (error) throw error;
      toast.success("Profile updated successfully!");
      setProfile({ ...profile, full_name: nameInput, mobile: mobileInput, class_level: classLevel, target_exam: targetExam });
      setIsEditing(false);
    } catch (error: any) {
      ...
      <main className="flex-1 p-4 space-y-6 pb-20 md:pb-6">
        ...
        <div className="bg-card rounded-2xl border border-border p-6 space-y-5">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-2">
              <Label className="flex items-center gap-2">
                <GraduationCap className="h-4 w-4 text-muted-foreground" /> Class
              </Label>
              {isEditing ? (
                ...
                <div className="h-10 px-3 py-2 rounded-md bg-muted border border-border text-sm text-muted-foreground">
                  {classLevel ? `Class ${classLevel}` : "Not set"}
                </div>
              )}
            </div>
            <div className="space-y-2">
              <Label className="flex items-center gap-2">
                <Target className="h-4 w-4 text-muted-foreground" /> Target Exam
              </Label>
              {isEditing ? (
                <Select value={targetExam} onValueChange={setTargetExam}>
                  ...
                  <SelectContent>
                    {["Board", "JEE", "NEET", "Foundation", "UPSC", "SSC", "Other"].map(e => (
          ...
          <div className="flex gap-3 pt-2">
            {isEditing ? (
              <>
                <Button variant="outline" onClick={() => { setNameInput(profile.full_name); setMobileInput(profile.mobile || ""); setClassLevel(profile.class_level || ""); setTargetExam(profile.target_exam || ""); setIsEditing(false); }} className="flex-1">Cancel</Button>
