...
const GOAL_OPTIONS = ["NEET", "JEE", "Board Exams", "Foundation", "Other"] as const;
...
const Signup = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { data: serverCheck, error } = await supabase.functions.invoke('validate-email', {
        body: { email: email.trim() },
      });
      if (!error && serverCheck?.blocked) {
    ...
    try {
      ...
      const result = await Promise.race([signup(email.trim(), password, name, goal, classGrade), timeoutPromise]);
      ...
      <div className="hidden lg:flex flex-1 bg-secondary items-center justify-center p-12">
        <div className="max-w-lg text-center text-secondary-foreground">
          <div className="w-24 h-24 mx-auto mb-8 bg-secondary-foreground/20 rounded-3xl flex items-center justify-center">
            <img src={logo} alt="Sadguru Classes" className="h-16 w-16 rounded-xl" />
          </div>
      ...
      <div className="flex-1 flex flex-col justify-center px-8 py-12 lg:px-16">
        <div className="mx-auto w-full max-w-md">
          <Link to="/" className="flex items-center gap-3 mb-8 lg:hidden">
            <img src={logo} alt="Sadguru Classes" className="h-12 w-12 rounded-xl" />
            <span className="font-bold text-2xl text-foreground">Sadguru Classes</span>
          </Link>

          <h1 className="text-3xl font-bold text-foreground mb-2">Create Account</h1>
          <p className="text-muted-foreground mb-8">
            Join Sadguru Classes today - Get instant access to free courses!
          </p>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-2">
                <Label>Class</Label>
                <Select value={classGrade} onValueChange={(v) => { setClassGrade(v); setErrorMessage(null); }}>
              ...
              <div className="space-y-2">
                <Label>Goal</Label>
                <Select value={goal} onValueChange={(v) => { setGoal(v); setErrorMessage(null); }}>
                  <SelectTrigger className="bg-background border-border h-12">
