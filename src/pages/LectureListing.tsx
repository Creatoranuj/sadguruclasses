...
const LectureListing = () => {
  ...
  // Fetch quizzes linked to lessons (DPP/TEST) AND standalone course-level quizzes
  useEffect(() => {
    if (!courseId) return;
  ...
  const rootLabel = fromParam === "courses" ? "Courses" : fromParam === "my-courses" ? "My Courses" : "All Classes";
  const rootHref = fromParam === "courses" ? "/courses" : fromParam === "my-courses" ? "/my-courses" : "/all-classes";
  const breadcrumbSegments = course ? [
    ...
    ...(parentChapter ? [
      { label: `${parentChapter.code} : ${parentChapter.title}`, href: `/classes/${courseId}/chapter/${parentChapter.id}${fromParam ? `?from=${fromParam}` : ""}` },
    ] : []),
    ...(chapter ? [
