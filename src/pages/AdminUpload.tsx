...
const AdminUpload = () => {
  ...
  const [useTelegramStorage, setUseTelegramStorage] = useState(false);
  ...
  // ─── Upload to Telegram ──────────────────────────────────────────────────
  const uploadToTelegram = async (file: File, fileName: string): Promise<string> => {
    ...
    const res = await fetch(`${supabaseUrl}/functions/v1/telegram-upload`, {
      method: 'POST',
    ...
    if (!res.ok || !data.success) {
      throw new Error(data.error || 'Telegram upload failed');
    }

    toast.success(`📲 PDF saved to Telegram (₹0 storage cost!)`);
    return data.telegram_url; // telegram://file_id
  };
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (uploadType !== "VIDEO" && pdfInputMode === "url") {
        ...
        // Check if we should use Telegram storage for non-video files
        if (useTelegramStorage && pdfFile.type === 'application/pdf') {
          contentUrl = await uploadToTelegram(pdfFile, pdfFile.name);
        } else {
      ...
      if (classPdfFile) {
        // Use Telegram for class PDF if toggle is on
        if (useTelegramStorage && classPdfFile.type === 'application/pdf') {
          classPdfFinalUrl = await uploadToTelegram(classPdfFile, classPdfFile.name);
        } else {
  ...
  const renderUploadForm = () => (
    <div className="space-y-5">
      ...
      {/* ── Telegram Storage Toggle ── */}
      <div className="flex items-center justify-between p-3 rounded-lg border border-border bg-muted/20">
        <div className="flex items-center gap-2">
          <span className="text-lg">📲</span>
          <div>
            <p className="text-sm font-medium text-foreground">Telegram Storage (Free)</p>
            <p className="text-xs text-muted-foreground">Save PDFs via Telegram Bot — ₹0 storage cost</p>
          </div>
        </div>
        <button
          onClick={() => setUseTelegramStorage(!useTelegramStorage)}
          className="p-1"
          title={useTelegramStorage ? "Using Telegram" : "Using Supabase Storage"}
        >
          {useTelegramStorage
            ? <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full bg-green-100 text-green-700">ON</span>
            : <span className="inline-flex items-center gap-1 text-xs font-medium px-2 py-1 rounded-full bg-muted text-muted-foreground">OFF</span>
          }
        </button>
      </div>
